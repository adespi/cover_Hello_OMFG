...
def double_pitch(file_name): #not used
  file_name_no_extension = os.path.splitext(file_name)[0]
  os.system("ffmpeg -y -i " + file_name_no_extension + "_x2.mp4 -af 'asetrate=44100*2/1,atempo=1/2' " + file_name_no_extension + "_x2_pitched_up.mp4")

def make_blink(x, name, repeat_every_x = 100):
  duration = x.duration
  CompositeVideoClip([x, clips_array([[white.set_duration(x.duration)]]).fx(vfx.blink, 0.15 , repeat_every_x - 0.15) ]).write_videofile("temp/"+name+".mp4") #removed x =
  return VideoFileClip("temp/"+name+".mp4").subclip(0, duration) #changed duration here

def make_black(x, name, repeat_every_x = 100):
  duration = x.duration
  CompositeVideoClip([black.set_duration(x.duration), clips_array([[x]]).fx(vfx.blink, 0.15 , repeat_every_x - 0.15) ]).write_videofile("temp/"+name+".mp4")
  return VideoFileClip("temp/"+name+".mp4").subclip(0, duration)
...
for i, x in enumerate(column(timings,0)):
  timings[i].append(VideoFileClip("temp/yamera/EWIS8487.MOV").audio_normalize().subclip(timings[i][1], timings[i][1] + timings[i][2] * mesured))
...
for i, x in enumerate(["a","b","c","chord1","chord2",'chord3','beat1','beat2','beat2_x2','beat2_x2_x2']):
  if (i == 2) :
    ...
    speed_coef_video = 0.513  #todo subclip (0, length)
  elif (i < 7) :
...
#melody_full_pitched_up = concatenate_videoclips([a, b, a, c]).speedx(2)
...
pause = clips_array([[beat2.crop(x2=length*4/5), c_x2.crop(x1=length*4/5)]]).subclip(0, mesured*0.5).volumex(0) #todo add pause c_x2 and beat2
...
set8 = make_blink(set8, "set8", melody_cropped_full_pitched_up.duration / 16) #.volumex(3) #todo increase only beat (or nothing) and add blink


#final_clip = concatenate_videoclips([set4, set5, set6, set7])
#final_clip.write_videofile("b4.mp4")

#set_sat.speedx(2).write_videofile("a.mp4")


final_clip = concatenate_videoclips([intro, set1, set2, set3, set4, set5, set6, set7, pause, set8, intro]) #todo intro at end is test
final_clip.write_videofile("out/hellov8.mp4")


exit()

#ffmpeg blink
#ffmpeg -i out/hellov5.mp4 -vf "movie=in/1920x1080-white-solid-color-background.jpg [logo]; 
#           [in][logo] overlay=x='if(lt(mod(t,0.6),0.5),NAN,0)':y=0 [out]"          -c:v h264 -c:a copy out/hellov5_temp.mp4 
